...
class IslesAugImgDataset(BaseDataset):
    def __init__(self, opt):
        ...
        save_path = '/home/jiamingzhao/ISLES2015/imgaug/train/'

        # 2. load_all modal into memory
        print('Loading ISLES Dataset ...')
        start = time()
        cache_path = os.path.join(data_root, 'aug_cache2D.pkl')
            ...
            for index in range(self.n_data):
                ...
                for modal in ['seg', 't1', 't1ce', 't2', 'flair']:
                    ...
                    for i in range(len(crop_list)):
                        if modal == 'seg':
                            modal_img = loader(modal_path, num=choose_slice_num, transform=segTrans, crop=crop_list[i])
                        else:
                            modal_img = loader(modal_path, num=choose_slice_num, transform=transform, crop=crop_list[i])
                        modal_img = cv2.resize(modal_img, (resize, resize))

                        # savetmp = save_path + modal + '/' + str(index*5+i+1) + '.png'
