...
from data.nii_data_loader import img_loader, load_set, normalize_img, seg_transform_img, findSegBox
import os
import os.path
import numpy as np
import cv2
...
class IslesImgDataset(BaseDataset):
    def __init__(self, opt):
        ...
        segTrans = seg_transform_img
        loader = img_loader
        choose_slice_num = 78  # change to maxSeg
        resize = 256        
        # self.trans = transforms.Normalize(0.5, 0.5)
        ...
        start = time()
        cache_path = os.path.join(data_root, 'fix_cache2D.pkl')
        if os.path.exists(cache_path):
            ...
            for index in range(self.n_data):
                ...
                for modal in ['seg', 't1', 't1ce', 't2', 'flair']:
                    ...
                    modal_img = None
                    if modal == 'seg':
                        modal_img = loader(modal_path, transform=segTrans)
                    else:
    ...
    def __getitem__(self, index):
        ...
        for modal in ['t1', 't1ce', 't2', 'flair']:
            ...
            # modal_tmp = self.trans(modal_tmp)
